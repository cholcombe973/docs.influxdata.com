...
The `room`, `token`, `global` and `state-changes-only` settings are optional.
If `room` is set to the relevant HipChat room and `token` is set to the API
access token you created in the section above, those settings serve as the
default `room` and `token` for alerts sent to HipChat.

If `global` is set to `true`, Kapacitor sends all alerts to HipChat without the
need to specify HipChat in TICKscripts.
If `state-changes-only` is set to `true`, Kapacitor only sends an alert to
